...
static void LVGL_LCD_FSMC_DMA_pCallback(DMA_HandleTypeDef *_hdma);
DMA_HandleTypeDef hdma_memtomem_dma2_channel4;

/**
  * Enable DMA controller clock
  * Configure DMA for memory to memory transfers
  *   hdma_memtomem_dma2_channel4
  */
void DMA2_Channel4_IRQHandler(void) {
    HAL_DMA_IRQHandler(&hdma_memtomem_dma2_channel4);
}
static void MX_DMA_Init(void)
{
  ...
  hdma_memtomem_dma2_channel4.Instance = DMA2_Channel4;
  hdma_memtomem_dma2_channel4.Init.Direction = DMA_MEMORY_TO_MEMORY;
  hdma_memtomem_dma2_channel4.Init.PeriphInc = DMA_PINC_ENABLE;
  hdma_memtomem_dma2_channel4.Init.MemInc = DMA_MINC_DISABLE;
  hdma_memtomem_dma2_channel4.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;
  hdma_memtomem_dma2_channel4.Init.MemDataAlignment = DMA_MDATAALIGN_HALFWORD;
  hdma_memtomem_dma2_channel4.Init.Mode = DMA_NORMAL;
  hdma_memtomem_dma2_channel4.Init.Priority = DMA_PRIORITY_HIGH;
	
	
  if (HAL_DMA_Init(&hdma_memtomem_dma2_channel4) != HAL_OK)
  {
//    HardFault_Handler();
  }
	   /*不受FreeRTOS调度*/
    ...
    HAL_NVIC_EnableIRQ(DMA2_Channel4_IRQn);
		...
		HAL_DMA_RegisterCallback(&hdma_memtomem_dma2_channel4, HAL_DMA_XFER_CPLT_CB_ID, LVGL_LCD_FSMC_DMA_pCallback);
...
//    lcd_draw_fast_rgb_color(area->x1,area->y1,area->x2,area->y2,(uint16_t*)color_p);
			...
			HAL_DMA_Start_IT(&hdma_memtomem_dma2_channel4, (uint32_t)color_p, (uint32_t)&LCD->LCD_RAM , ((area->x2+1) - area->x1) * ((area->y2+1) - area->y1)); //写数据
#endif
    /* 重要!!!
     * 通知图形库，已经刷新完毕了 */
